...
let RenderedContentHoverParts = class RenderedContentHoverParts extends Disposable {
    ...
    getAccessibleHoverContentAtIndex(index) {
        ...
        if (renderedPart.type === 'statusBar') {
            const statusBarDescription = [localize("vs/editor/contrib/hover/browser/contentHoverRendered",0, "This is a hover status bar.")];
            for (const action of renderedPart.actions) {
                const keybinding = action.actionKeybindingLabel;
                if (keybinding) {
                    statusBarDescription.push(localize("vs/editor/contrib/hover/browser/contentHoverRendered",1, "It has an action with label {0} and keybinding {1}.", action.actionLabel, keybinding));
                }
                else {
                    statusBarDescription.push(localize("vs/editor/contrib/hover/browser/contentHoverRendered",2, "It has an action with label {0}.", action.actionLabel));
                }
            }
