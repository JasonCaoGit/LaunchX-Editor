...
const $ = dom.$;
const increaseHoverVerbosityIcon = registerIcon('hover-increase-verbosity', Codicon.add, nls.localize("vs/editor/contrib/hover/browser/markdownHoverParticipant",0, 'Icon for increaseing hover verbosity.'));
const decreaseHoverVerbosityIcon = registerIcon('hover-decrease-verbosity', Codicon.remove, nls.localize("vs/editor/contrib/hover/browser/markdownHoverParticipant",1, 'Icon for decreasing hover verbosity.'));
...
let MarkdownHoverParticipant = class MarkdownHoverParticipant {
    ...
    createLoadingMessage(anchor) {
        return new MarkdownHover(this, anchor.range, [new MarkdownString().appendText(nls.localize("vs/editor/contrib/hover/browser/markdownHoverParticipant",2, "Loading..."))], false, 2000);
    }
    computeSync(anchor, lineDecorations) {
        ...
        if (stopRenderingLineAfter >= 0 && lineLength > stopRenderingLineAfter && anchor.range.startColumn >= stopRenderingLineAfter) {
            ...
            result.push(new MarkdownHover(this, anchor.range, [{
                    value: nls.localize("vs/editor/contrib/hover/browser/markdownHoverParticipant",3, "Rendering paused for long line for performance reasons. This can be configured via `editor.stopRenderingLineAfter`.")
                }], false, index++));
        }
        if (!stopRenderingMessage && typeof maxTokenizationLineLength === 'number' && lineLength >= maxTokenizationLineLength) {
            result.push(new MarkdownHover(this, anchor.range, [{
                    value: nls.localize("vs/editor/contrib/hover/browser/markdownHoverParticipant",4, "Tokenization is skipped for long lines for performance reasons. This can be configured via `editor.maxTokenizationLineLength`.")
                }], false, index++));
        }
...
export function labelForHoverVerbosityAction(keybindingService, action) {
    switch (action) {
        case HoverVerbosityAction.Increase: {
            ...
            return kb ?
                nls.localize("vs/editor/contrib/hover/browser/markdownHoverParticipant",5, "Increase Hover Verbosity ({0})", kb.getLabel()) :
                nls.localize("vs/editor/contrib/hover/browser/markdownHoverParticipant",6, "Increase Hover Verbosity");
        }
        case HoverVerbosityAction.Decrease: {
            const kb = keybindingService.lookupKeybinding(DECREASE_HOVER_VERBOSITY_ACTION_ID);
            return kb ?
                nls.localize("vs/editor/contrib/hover/browser/markdownHoverParticipant",7, "Decrease Hover Verbosity ({0})", kb.getLabel()) :
                nls.localize("vs/editor/contrib/hover/browser/markdownHoverParticipant",8, "Decrease Hover Verbosity");
        }
    }
}
