    ...
    computeAsync(anchor, _lineDecorations, source, token) {
        ...
        return new AsyncIterableObject(async (executor) => {
            ...
            if (isNonEmptyArray(part.item.hint.textEdits)) {
                executor.emitOne(new MarkdownHover(this, anchor.range, [new MarkdownString().appendText(localize("vs/editor/contrib/inlayHints/browser/inlayHintsHover",0, "Double-click to insert"))], false, 10001));
            }
            ...
            if (part.part.location || part.part.command) {
                ...
                const kb = useMetaKey
                    ? platform.isMacintosh
                        ? localize("vs/editor/contrib/inlayHints/browser/inlayHintsHover",1, "cmd + click")
                        : localize("vs/editor/contrib/inlayHints/browser/inlayHintsHover",2, "ctrl + click")
                    : platform.isMacintosh
                        ? localize("vs/editor/contrib/inlayHints/browser/inlayHintsHover",3, "option + click")
                        : localize("vs/editor/contrib/inlayHints/browser/inlayHintsHover",4, "alt + click");
                if (part.part.location && part.part.command) {
                    linkHint = new MarkdownString().appendText(localize("vs/editor/contrib/inlayHints/browser/inlayHintsHover",5, 'Go to Definition ({0}), right click for more', kb));
                }
                else if (part.part.location) {
                    linkHint = new MarkdownString().appendText(localize("vs/editor/contrib/inlayHints/browser/inlayHintsHover",6, 'Go to Definition ({0})', kb));
                }
                else if (part.part.command) {
                    linkHint = new MarkdownString(`[${localize("vs/editor/contrib/inlayHints/browser/inlayHintsHover",7, "Execute Command")}](${asCommandLink(part.part.command)} "${part.part.command.title}") (${kb})`, { isTrusted: true });
                }
                if (linkHint) {
