...
export const pasteWidgetVisibleCtx = new RawContextKey('pasteWidgetVisible', false, localize("vs/editor/contrib/dropOrPasteInto/browser/copyPasteController",0, "Whether the paste widget is showing"));
...
let CopyPasteController = class CopyPasteController extends Disposable {
    ...
    constructor(editor, instantiationService, _bulkEditService, _clipboardService, _commandService, _configService, _languageFeaturesService, _quickInputService, _progressService) {
        ...
        this._postPasteWidgetManager = this._register(instantiationService.createInstance(PostEditWidgetManager, 'pasteIntoEditor', editor, pasteWidgetVisibleCtx, { id: changePasteTypeCommandId, label: localize("vs/editor/contrib/dropOrPasteInto/browser/copyPasteController",1, "Show paste options...") }, () => CopyPasteController_1._configureDefaultAction ? [CopyPasteController_1._configureDefaultAction] : []));
    }
    changePasteType() {
    ...
    showPasteAsNoEditMessage(selections, preference) {
        const kindLabel = 'only' in preference
            ...
            : 'preferences' in preference
                ? (preference.preferences.length ? preference.preferences.map(preference => preference.value).join(', ') : localize("vs/editor/contrib/dropOrPasteInto/browser/copyPasteController",2, "empty"))
                : preference.providerId;
        MessageController.get(this._editor)?.showMessage(localize("vs/editor/contrib/dropOrPasteInto/browser/copyPasteController",3, "No paste edits for '{0}' found", kindLabel), selections[0].getStartPosition());
    }
    doPasteInline(allProviders, selections, dataTransfer, metadata, clipboardEvent) {
                        ...
                        return new Promise((resolve, reject) => {
                            (async () => {
                                try {
                                    ...
                                    const resolved = resolveP && await this._pasteProgressManager.showWhile(selections[0].getEndPosition(), localize("vs/editor/contrib/dropOrPasteInto/browser/copyPasteController",4, "Resolving paste edit. Click to cancel"), Promise.race([showP.p, resolveP]), {
                                        cancel: () => {
                                            showP.cancel();
        ...
        this._pasteProgressManager.showWhile(selections[0].getEndPosition(), localize("vs/editor/contrib/dropOrPasteInto/browser/copyPasteController",5, "Running paste handlers. Click to cancel and do basic paste"), p, {
            cancel: async () => {
                try {
                    p.cancel();
    ...
    showPasteAsPick(preference, allProviders, selections, dataTransfer, metadata) {
            ...
            try {
                ...
                if (preference) {
                    ...
                    const configureDefaultItem = {
                        ...
                        label: localize("vs/editor/contrib/dropOrPasteInto/browser/copyPasteController",6, "Configure default paste action"),
                        edit: undefined,
                    };
                    const selected = await this._quickInputService.pick([
                        ...
                        placeHolder: localize("vs/editor/contrib/dropOrPasteInto/browser/copyPasteController",7, "Select Paste Action"),
                    });
        ...
        this._progressService.withProgress({
            ...
            title: localize("vs/editor/contrib/dropOrPasteInto/browser/copyPasteController",8, "Running paste handlers"),
        }, () => p);
    }
