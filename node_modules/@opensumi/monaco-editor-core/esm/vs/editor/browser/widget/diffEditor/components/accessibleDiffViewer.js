...
const accessibleDiffViewerInsertIcon = registerIcon('diff-review-insert', Codicon.add, localize("vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer",0, 'Icon for \'Insert\' in accessible diff viewer.'));
const accessibleDiffViewerRemoveIcon = registerIcon('diff-review-remove', Codicon.remove, localize("vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer",1, 'Icon for \'Remove\' in accessible diff viewer.'));
const accessibleDiffViewerCloseIcon = registerIcon('diff-review-close', Codicon.close, localize("vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer",2, 'Icon for \'Close\' in accessible diff viewer.'));
...
let View = class View extends Disposable {
    constructor(_element, _model, _width, _height, _models, _languageService) {
        ...
        this._register(autorun(reader => {
            ...
            if (this._model.canClose.read(reader)) {
                this._actionBar.push(new Action('diffreview.close', localize("vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer",3, "Close"), 'close-diff-review ' + ThemeIcon.asClassName(accessibleDiffViewerCloseIcon), true, async () => _model.close()), { label: false, icon: true });
            }
        }));
    ...
    _render(store) {
        ...
        container.setAttribute('aria-label', localize("vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer",4, 'Accessible Diff Viewer. Use arrow up and down to navigate.'));
        ...
        for (const viewItem of group?.lines || []) {
            ...
            if (viewItem.type === LineType.Header) {
                ...
                const getAriaLines = (lines) => lines === 0 ? localize("vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer",5, "no lines changed")
                    : lines === 1 ? localize("vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer",6, "1 line changed")
                        : localize("vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer",7, "{0} lines changed", lines);
                ...
                header.setAttribute('aria-label', localize("vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer",8, "Difference {0} of {1}: original line {2}, {3}, modified line {4}, {5}", (diffIndex + 1), diffsLength, r.original.startLineNumber, originalChangedLinesCntAria, r.modified.startLineNumber, modifiedChangedLinesCntAria));
    ...
    _createRow(item, lineHeight, width, originalOptions, originalModel, originalModelOpts, modifiedOptions, modifiedModel, modifiedModelOpts) {
        ...
        if (lineContent.length === 0) {
            lineContent = localize("vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer",9, "blank");
        }
        let ariaLabel = '';
        switch (item.type) {
            case LineType.Unchanged:
                if (item.originalLineNumber === item.modifiedLineNumber) {
                    ariaLabel = localize("vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer",10, "{0} unchanged line {1}", lineContent, item.originalLineNumber);
                }
                else {
                    ariaLabel = localize("vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer",11, "{0} original line {1} modified line {2}", lineContent, item.originalLineNumber, item.modifiedLineNumber);
                }
                break;
            case LineType.Added:
                ariaLabel = localize("vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer",12, "+ {0} modified line {1}", lineContent, item.modifiedLineNumber);
                break;
            case LineType.Deleted:
                ariaLabel = localize("vs/editor/browser/widget/diffEditor/components/accessibleDiffViewer",13, "- {0} original line {1}", lineContent, item.originalLineNumber);
                break;
        }
