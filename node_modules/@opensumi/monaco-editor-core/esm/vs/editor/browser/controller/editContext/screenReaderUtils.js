...
export function ariaLabelForScreenReaderContent(options, keybindingService) {
    ...
    if (accessibilitySupport === 1 /* AccessibilitySupport.Disabled */) {
        ...
        const editorNotAccessibleMessage = nls.localize("vs/editor/browser/controller/editContext/screenReaderUtils",0, "The editor is not accessible at this time.");
        if (toggleKeybindingLabel) {
            return nls.localize("vs/editor/browser/controller/editContext/screenReaderUtils",1, "{0} To enable screen reader optimized mode, use {1}", editorNotAccessibleMessage, toggleKeybindingLabel);
        }
        else if (runCommandKeybindingLabel) {
            return nls.localize("vs/editor/browser/controller/editContext/screenReaderUtils",2, "{0} To enable screen reader optimized mode, open the quick pick with {1} and run the command Toggle Screen Reader Accessibility Mode, which is currently not triggerable via keyboard.", editorNotAccessibleMessage, runCommandKeybindingLabel);
        }
        else if (keybindingEditorKeybindingLabel) {
            return nls.localize("vs/editor/browser/controller/editContext/screenReaderUtils",3, "{0} Please assign a keybinding for the command Toggle Screen Reader Accessibility Mode by accessing the keybindings editor with {1} and run it.", editorNotAccessibleMessage, keybindingEditorKeybindingLabel);
        }
        else {
            // SOS
