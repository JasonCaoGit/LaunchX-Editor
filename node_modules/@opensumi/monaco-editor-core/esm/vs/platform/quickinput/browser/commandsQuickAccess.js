...
let AbstractCommandsQuickAccessProvider = class AbstractCommandsQuickAccessProvider extends PickerQuickAccessProvider {
    ...
    async _getPicks(filter, _disposables, token, runOptions) {
        ...
        for (let i = 0; i < filteredCommandPicks.length; i++) {
            ...
            if (i === 0 && this.commandsHistory.peek(commandPick.commandId)) {
                commandPicks.push({ type: 'separator', label: localize("vs/platform/quickinput/browser/commandsQuickAccess",0, "recently used") });
                addOtherSeparator = true;
            }
            if (addSuggestedSeparator && commandPick.tfIdfScore !== undefined) {
                commandPicks.push({ type: 'separator', label: localize("vs/platform/quickinput/browser/commandsQuickAccess",1, "similar commands") });
            ...
            if (addCommonlyUsedSeparator && commandPick.tfIdfScore === undefined && !this.commandsHistory.peek(commandPick.commandId) && this.options.suggestedCommandIds?.has(commandPick.commandId)) {
                commandPicks.push({ type: 'separator', label: localize("vs/platform/quickinput/browser/commandsQuickAccess",2, "commonly used") });
                addOtherSeparator = true;
            ...
            if (addOtherSeparator && commandPick.tfIdfScore === undefined && !this.commandsHistory.peek(commandPick.commandId) && !this.options.suggestedCommandIds?.has(commandPick.commandId)) {
                commandPicks.push({ type: 'separator', label: localize("vs/platform/quickinput/browser/commandsQuickAccess",3, "other commands") });
        ...
        return {
            ...
            additionalPicks: (async () => {
                ...
                if (addSuggestedSeparator && commandPicks[0]?.type !== 'separator') {
                    commandPicks.unshift({ type: 'separator', label: localize("vs/platform/quickinput/browser/commandsQuickAccess",4, "similar commands") });
                }
                return commandPicks;
            })()
        };
    ...
    toCommandPick(commandPick, runOptions) {
        ...
        const ariaLabel = keybinding ?
            localize("vs/platform/quickinput/browser/commandsQuickAccess",5, "{0}, {1}", commandPick.label, keybinding.getAriaLabel()) :
            commandPick.label;
        return {
            ...commandPick,
            ariaLabel,
            ...
            accept: async () => {
                ...
                try {
                    ...
                    if (!isCancellationError(error)) {
                        this.dialogService.error(localize("vs/platform/quickinput/browser/commandsQuickAccess",6, "Command '{0}' resulted in an error", commandPick.label), toErrorMessage(error));
                    }
                }
            }
        };
