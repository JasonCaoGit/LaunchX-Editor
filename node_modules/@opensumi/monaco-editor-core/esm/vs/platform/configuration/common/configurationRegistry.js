...
class ConfigurationRegistry {
    constructor() {
        ...
        this.defaultLanguageConfigurationOverridesNode = {
            id: 'defaultOverrides',
            title: nls.localize("vs/platform/configuration/common/configurationRegistry",0, "Default Language Configuration Overrides"),
            properties: {}
        };
    ...
    updateDefaultOverrideProperty(key, newDefaultOverride, source) {
        const property = {
            ...
            description: nls.localize("vs/platform/configuration/common/configurationRegistry",1, "Configure settings to be overridden for the {0} language.", getLanguageTagSettingPlainKey(key)),
    ...
    updateOverridePropertyPatternKey() {
        for (const overrideIdentifier of this.overrideIdentifiers.values()) {
            ...
            const resourceLanguagePropertiesSchema = {
                type: 'object',
                description: nls.localize("vs/platform/configuration/common/configurationRegistry",2, "Configure editor settings to be overridden for a language."),
                errorMessage: nls.localize("vs/platform/configuration/common/configurationRegistry",3, "This setting does not support per-language configuration."),
                $ref: resourceLanguageSettingsSchemaId,
            };
    ...
    registerOverridePropertyPatternKey() {
        const resourceLanguagePropertiesSchema = {
            type: 'object',
            description: nls.localize("vs/platform/configuration/common/configurationRegistry",4, "Configure editor settings to be overridden for a language."),
            errorMessage: nls.localize("vs/platform/configuration/common/configurationRegistry",5, "This setting does not support per-language configuration."),
            $ref: resourceLanguageSettingsSchemaId,
        };
...
export function validateProperty(property, schema) {
    if (!property.trim()) {
        return nls.localize("vs/platform/configuration/common/configurationRegistry",6, "Cannot register an empty property");
    }
    if (OVERRIDE_PROPERTY_REGEX.test(property)) {
        return nls.localize("vs/platform/configuration/common/configurationRegistry",7, "Cannot register '{0}'. This matches property pattern '\\\\[.*\\\\]$' for describing language specific editor settings. Use 'configurationDefaults' contribution.", property);
    }
    if (configurationRegistry.getConfigurationProperties()[property] !== undefined) {
        return nls.localize("vs/platform/configuration/common/configurationRegistry",8, "Cannot register '{0}'. This property is already registered.", property);
    }
    if (schema.policy?.name && configurationRegistry.getPolicyConfigurations().get(schema.policy?.name) !== undefined) {
        return nls.localize("vs/platform/configuration/common/configurationRegistry",9, "Cannot register '{0}'. The associated policy {1} is already registered with {2}.", property, schema.policy?.name, configurationRegistry.getPolicyConfigurations().get(schema.policy?.name));
    }
    return null;
}
