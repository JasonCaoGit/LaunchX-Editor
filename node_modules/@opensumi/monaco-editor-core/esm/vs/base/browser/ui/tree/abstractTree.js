...
class FindWidget extends Disposable {
    ...
    constructor(container, tree, contextViewProvider, placeholder, toggleContributions = [], options) {
        ...
        this.findInput = this._register(new FindInput(this.elements.findInput, contextViewProvider, {
            label: localize("vs/base/browser/ui/tree/abstractTree",0, "Type to search"),
            placeholder,
        ...
        const closeAction = this._register(new Action('close', localize("vs/base/browser/ui/tree/abstractTree",1, "Close"), 'codicon codicon-close', true, () => this.dispose()));
...
export class AbstractFindController {
    ...
    constructor(tree, filter, contextViewProvider, options = {}) {
        ...
        this._placeholder = options.placeholder ?? localize("vs/base/browser/ui/tree/abstractTree",2, "Type to search");
    }
    isOpened() {
        return !!this.widget;
    }
    ...
    renderMessage(showNotFound, warningMessage) {
        if (showNotFound) {
            if (this.tree.options.showNotFoundMessage ?? true) {
                this.widget?.showMessage({ type: 2 /* MessageType.WARNING */, content: warningMessage ?? localize("vs/base/browser/ui/tree/abstractTree",3, "No results found.") });
            }
            else {
    ...
    alertResults(results) {
        if (!results) {
            alert(localize("vs/base/browser/ui/tree/abstractTree",4, "No results"));
        }
        else {
            alert(localize("vs/base/browser/ui/tree/abstractTree",5, "{0} results", results));
        }
    }
    dispose() {
...
export class FindController extends AbstractFindController {
    ...
    set mode(mode) {
        ...
        this.placeholder = isFilterMode ? localize("vs/base/browser/ui/tree/abstractTree",6, "Type to filter") : localize("vs/base/browser/ui/tree/abstractTree",7, "Type to search");
    ...
    constructor(tree, filter, contextViewProvider, options = {}) {
        ...
        const toggleContributions = [{
                ...
                title: localize("vs/base/browser/ui/tree/abstractTree",8, "Filter"),
                isChecked: defaultFindMode === TreeFindMode.Filter,
            }, {
                id: DefaultTreeToggles.MatchType,
                icon: Codicon.searchFuzzy,
                title: localize("vs/base/browser/ui/tree/abstractTree",9, "Fuzzy Match"),
