...
export class InputBox extends Widget {
    ...
    _showMessage() {
        ...
        if (this.message.type === 3 /* MessageType.ERROR */) {
            alertText = nls.localize("vs/base/browser/ui/inputbox/inputBox",0, "Error: {0}", this.message.content);
        }
        else if (this.message.type === 2 /* MessageType.WARNING */) {
            alertText = nls.localize("vs/base/browser/ui/inputbox/inputBox",1, "Warning: {0}", this.message.content);
        }
        else {
            alertText = nls.localize("vs/base/browser/ui/inputbox/inputBox",2, "Info: {0}", this.message.content);
        }
...
export class HistoryInputBox extends InputBox {
    constructor(container, contextViewProvider, options) {
        const NLS_PLACEHOLDER_HISTORY_HINT_SUFFIX_NO_PARENS = nls.localize("vs/base/browser/ui/inputbox/inputBox",3, ' or {0} for history', `\u21C5`);



        const NLS_PLACEHOLDER_HISTORY_HINT_SUFFIX_IN_PARENS = nls.localize("vs/base/browser/ui/inputbox/inputBox",4, ' ({0} for history)', `\u21C5`);
    ...
    showNextValue() {
        ...
        this.value = next ?? '';
        aria.status(this.value ? this.value : nls.localize("vs/base/browser/ui/inputbox/inputBox",5, "Cleared Input"));
    }
    showPreviousValue() {
        if (!this.history.has(this.value)) {
            this.addToHistory();
